...
//!
//! `infra-net-libp2p` has the same file, for the same reason, and the two are
//! deliberately separate. A crate's `#[cfg(test)]` modules are not reachable
//! from another crate, and the alternative — a shared test-support crate — is a
//! dependency `app` would carry in order to run tests. That is the same trade
//! `test_peers.rs` already makes in four crates: fifty lines duplicated beats a
//! coupling the architecture does not want.
//!
//! # The problem it removes
//!
//! The composition-root smoke tests return early and green when a swarm cannot
//! start. That is defensible in a sandbox with no network namespace: it is a
//! fact about the machine, not a failure of the code. It is *not* defensible on
//! a machine that has networking, because a skip there is a test that proved
