...
2. **Read and analyze the existing prompt**
   ...
   - Norms
   - Safeguards
   - Agents (REQUIRED — subagent roster + orchestration)
     c. Understand the current architecture, entities, and constraints
     d. **If `## Agents` is missing or Operations changed**, backfill/refresh the section. ALWAYS confirm each agent assignment via AskUserQuestion with: a recommended agent + justification, 1–2 alternatives, and an "other" option. Never auto-assign even when the choice seems obvious. Recommendations should come from the role roster: `domain-modeler` (domain types, ports, shared_types events), `api-designer` (Axum routes, HTTP DTOs, `bin/{ctx}_http.rs`), `repo` (DynamoDB adapters and shared DDB infra), `application-handler` (commands/queries/services, context wiring, event/scheduled Lambdas, non-DDB adapters), `test-writer` (all `*_test.rs` plus `tests/integration/src/*_flow.rs`), and `spdd-executor` as the fallback for purely mechanical cross-cutting work driven by a settled canvas.
...
- **ALWAYS use the AskUserQuestion tool when anything is ambiguous** — never silently assume. If the update instruction is open to interpretation, the affected section(s) are unclear, or the change conflicts with adjacent sections, stop and ask via AskUserQuestion before editing the prompt.
...
- Preserve the REASONS Canvas structure (all 7 sections must remain) **plus the required `## Agents` section** — never drop Agents during an update; if Operations change, update Agents to keep coverage.
