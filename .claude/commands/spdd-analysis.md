...
- **ALWAYS use the AskUserQuestion tool when anything is ambiguous** — never silently assume. If a requirement is unclear, scope is fuzzy, a domain concept could be interpreted multiple ways, or an AC has gaps, stop and ask via AskUserQuestion before proceeding. Surfacing ambiguity in the Risk & Gap section is not a substitute for asking when the answer is needed to continue the analysis.
