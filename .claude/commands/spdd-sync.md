...
2. **Read and parse the structured prompt file**
   ...
   | **Agents**           | Subagent roster + orchestration | Medium (re-sync whenever Operations or Structure shift across layers); if missing, backfill it |
...
- **ALWAYS use the AskUserQuestion tool when anything is ambiguous** — never silently assume. If a code change could map to multiple sections, the intent behind a refactor is unclear, or it is uncertain whether the prompt should follow the code (or vice-versa), stop and ask via AskUserQuestion before editing the prompt.
- Do NOT remove content from prompt without explicit user approval
- **Never drop the `## Agents` section**. If it does not exist in the prompt being synced, treat that as a gap and backfill it. ALWAYS confirm each agent assignment via AskUserQuestion with a recommended agent + justification, 1–2 alternatives, and an "other" option — do not auto-assign even when an Operation cleanly maps to a single specialty. Keep agent assignments in step with any Operations changes. Draw recommendations from the trading_agent role roster: `domain-modeler`, `api-designer`, `repo`, `application-handler`, `test-writer`, with `spdd-executor` as the fallback for purely mechanical canvas-driven work.
