...
2. **Read and parse the structured prompt file**
   ...
   | **Agents**           | Subagent roster + orchestration | **Delegate each Operation to its assigned subagent via the Agent tool** |
...
- **ALWAYS use the AskUserQuestion tool when anything is ambiguous** — never silently assume. If a prompt section is unclear, an Operation's intent could be implemented multiple ways, a method signature/field name/error message conflicts across sections, or a Norm doesn't cover the case at hand, stop and ask via AskUserQuestion before generating code. If the prompt itself is the source of ambiguity, surface it and propose a `/spdd-prompt-update` rather than guessing.
- Do NOT generate code without first reading the entire prompt file
- **Agents section is REQUIRED**: refuse to generate if `## Agents` is missing or does not cover every Operation. Surface the gap and direct the user to `/spdd-prompt-update` to add the section.
- **Delegate per the Agents section**: invoke the named subagent (via the Agent tool) for each Operation rather than implementing it inline. Follow the Orchestration subsection for ordering and hand-offs.
