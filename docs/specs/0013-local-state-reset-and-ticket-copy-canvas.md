...
**Status:** approved target, not implemented. Subordinate to `AGENTS.md` and to the system canvas `0002`, which this one **amends in one place** (safeguard S8 — see §7/S5 below).
...
├── ports/port_fakes.rs                       # fakes implement the new method
...
├── store_fs/src/stores/file_peer_cache.rs        # regression test only
...
tests/integration/                            # A1, A2, A3, A4, A6
```
...
**S5 — Safeguard S8 of canvas `0002` is contradicted by the clipboard, and must be amended before this ships. `⚠ AMENDS 0002`**
...
The honest resolution is small: a sentence in S8 and a fourth entry in `Usage::DISCLOSURES`, saying that copying a join ticket puts this peer's addresses on the system clipboard and that a syncing clipboard manager will carry them off the machine. It is `$spdd-prompt-update` work against `0002`, **its own commit**, and this canvas does not assume it has happened.

**S6 — Replay is re-armed by a clear, and this is stated rather than discovered.** A cleared `AuthorLog` will accept a message it has already applied, so `0002`'s AC7 (exactly-once) is scoped to a run rather than to an identity. Every signature is still verified and every author policy still applies, so the exposure is redundant display, not forged content. `0002`'s AC7 gains a clause noting the clear boundary in the same `$spdd-prompt-update` as S5.
...
- **S5 and S6 are amendments to `0002` awaiting `$spdd-prompt-update`.** They are recorded here so the reviewer sees them, not so this canvas can make them.
...
- **`ClearedHistory`'s two counts are for the notice.** If A8's wording ends up needing only one of them, drop the other rather than reporting a number nobody reads.
