...
**Q1 — Clipboard mechanism (E8). Should be decided before the canvas, not inside it.** It determines whether a dependency enters `app`, whether the feature works over SSH, and whether A11 is satisfiable by detection or only by careful wording (R6). *Recommendation:* OSC 52, with the notice worded as an attempt rather than a success. It adds nothing to the dependency graph, works in the terminal-over-SSH case this project's users are likely to be in, and its weakness is a wording problem rather than a portability problem. A native crate is the better answer only if the desktop root arrives and wants to share the mechanism — and per the target layout it must not share it anyway.

**Q2 — What does "clear cached peers" do about live sessions (E5, R2)?** Three coherent answers: (a) forget only peers with no live session, leaving connected ones — honest, and leaves a non-empty list after a "clear"; (b) forget everything and close every session, which is `leave` plus forgetting, and makes the emptiness real; (c) refuse while any session is live and tell the user to leave first. *Recommendation:* (b) — the user asked for the cache to be empty, and (a) delivers a list that still has rows in it while claiming to be cleared.

**Q3 — Does clearing chats clear dedup state, or only what is displayed (E3, R4)?** Clearing only the display keeps AC7 intact across the clear but leaves the process holding a growing set of marks the user believes they erased — and "clear chats" that leaves the peer refusing to re-show a message it will not display is confusing in its own way. *Recommendation:* clear the author logs with the history. It is what "clear" means, the re-arming is bounded to whatever arrives afterwards, and every signature is still verified.

**Q4 — Confirmation shape (R8).** A second keystroke, a modal overlay, or a typed word. *Recommendation:* a confirmation overlay reusing the existing `Overlay` mechanism, since it is the only one of the three that can say what is about to be destroyed before it is.

**Q5 — Does S8 need amending for the clipboard (R5)?** This is the one item that is not the canvas author's call to make quietly. Either the disclosure gains a clipboard sentence, or the copy feature must not use the system clipboard.
...
Nothing here crosses a context boundary, so no `system-architect` review is required before the canvas. Q1 and Q5 are the two answers the canvas cannot supply for itself.
